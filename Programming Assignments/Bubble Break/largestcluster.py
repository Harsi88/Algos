...
#    matrix = board
    ...
    for i in range(len(matrix)):
        for j in range(len(matrix[0])):
            if(backup[i][j]!=0):
                count = cluster(backup,i,j,backup[i][j],0)
                if(count > 1):
                    temp = [count, color_count[matrix[i][j]], j, i, matrix[i][j]]   #size of cluster
                    cluster_loc.append(temp)
    ...
    cluster_loc.reverse()   # for largest cluster first
   
#    temp = []
#    temp = cluster_loc[0]
#    largest_count = cluster_loc[0][0]
#    i=0;
#   while(cluster_loc[i][0]==largest_count):
#        temp = cluster_loc[i][0]
#        i+=1
    
    burst(matrix,cluster_loc[0][3],cluster_loc[0][2],cluster_loc[0][4])
#    burst(matrix,temp[3],temp[2],temp[4])   #for bottom first
    
    shift(matrix)    
    L1.append([numpy.matrix(matrix),cluster_loc[0][0]**2])
    #---
    solve1(matrix)
    return L1
...
for i in range(iteration):
    ...
    t = time.time() - t
    avg_rand_score += s[-1]
    plots.append([counter,s[-1]])
    avg_1_score += l[-1]
    plotl.append([counter,l[-1]])
...
plot1 = Gnuplot.PlotItems.Data(plots,title="")
g.plot(plot1)
"""
