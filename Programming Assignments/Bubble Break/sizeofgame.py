...
#    matrix = board
    ...
    if cluster_loc==[]:
        ...
        return L    
    cluster_loc.sort()
    num_clusters.append(len(cluster_loc))
...
#    matrix = board
    ...
    for i in range(len(matrix)):
        for j in range(len(matrix[0])):
            if(backup[i][j]!=0):
                count = cluster(backup,i,j,backup[i][j],0)
                if(count > 1):
                    temp = [color_count[matrix[i][j]],i,j,count,matrix[i][j]]
                    cluster_loc.append(temp)
    ...
    if cluster_loc==[]:
        ...
        return L1    
    cluster_loc.sort()
    num_clusters1.append(len(cluster_loc))
...
#    print s[-1],len(s),l[-1],len(l),t
    ...
    for j in range(0,len(num_clusters)):
        total_possibilities *= num_clusters[j]
    temp = len(l)**(sum(num_clusters1)/len(l))
    print math.log10(temp)
    print temp
